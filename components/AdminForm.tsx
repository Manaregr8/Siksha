  ...
  return (
    <div className="space-y-8">
      <div className="rounded-xl border border-black/5 bg-white p-5 shadow-md dark:border-white/10 dark:bg-neutral-900">
        <h2 className="text-base font-semibold text-neutral-900 dark:text-white">Overview</h2>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section className="rounded-xl border border-black/5 bg-white p-5 shadow-md dark:border-white/10 dark:bg-neutral-900">
          <h2 className="text-base font-semibold text-neutral-900 dark:text-white">Add college</h2>
        ...
        <section className="rounded-xl border border-black/5 bg-white p-5 shadow-md dark:border-white/10 dark:bg-neutral-900">
          <h2 className="text-base font-semibold text-neutral-900 dark:text-white">Add course</h2>
          <div className="mt-4 grid grid-cols-1 gap-3">
      ...
      <section className="rounded-xl border border-black/5 bg-white p-5 shadow-md dark:border-white/10 dark:bg-neutral-900">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-base font-semibold text-neutral-900 dark:text-white">Manage colleges</h2>
          <button
            type="button"
            onClick={() => refresh().catch(() => setMessage("Failed to refresh"))}
            className="rounded-xl border border-black/10 px-3 py-1.5 text-sm text-neutral-700 hover:bg-neutral-50 dark:border-white/10 dark:text-neutral-200 dark:hover:bg-neutral-800"
          >
            Refresh
          </button>
        </div>
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs text-neutral-600 dark:text-neutral-300">
              <tr>
            ...
            <tbody className="text-neutral-800 dark:text-neutral-100">
              {colleges.map((c) => (
                <tr key={c.id} className="border-t border-black/5 dark:border-white/10">
                  ...
                  <td className="py-3 pr-4 font-mono text-xs text-neutral-600 dark:text-neutral-300">{c.id}</td>
                  <td className="py-3">
                    <button
                      type="button"
                      onClick={() => deleteCollege(c.id).catch((e) => setMessage(e.message))}
                      className="rounded-xl bg-neutral-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-neutral-800 dark:bg-white dark:text-neutral-900 dark:hover:bg-neutral-200"
                    >
                      Delete
                    </button>
                  </td>
      ...
      <section className="rounded-xl border border-black/5 bg-white p-5 shadow-md dark:border-white/10 dark:bg-neutral-900">
        <h2 className="text-base font-semibold text-neutral-900 dark:text-white">Manage courses</h2>
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs text-neutral-600 dark:text-neutral-300">
              <tr>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-black/5 bg-neutral-50 p-4 dark:border-white/10 dark:bg-neutral-950">
      <div className="text-xs font-medium text-neutral-600 dark:text-neutral-300">{label}</div>
...
function Input({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <label className="block">
      <div className="mb-1 text-xs font-medium text-neutral-600 dark:text-neutral-300">{label}</div>
      <input
        value={value}
  ...
  return (
    <label className="block">
      ...
      <textarea
        ...
        onChange={(e) => onChange(e.target.value)}
        rows={3}
        className="w-full rounded-xl border border-black/5 bg-white px-3 py-2 text-sm text-neutral-900 outline-none dark:border-white/10 dark:bg-neutral-950 dark:text-white"
      />
    </label>
  );
}
